...
`.env.local`을 직접 만들고 Supabase project URL과 publishable/anon key만 입력한다. 이 저장소에는 환경변수 예시 파일을 두지 않는다. DB password, service role key, Google Client Secret은 프론트엔드 환경변수로 사용하지 않는다.
...
- Phase 3 production 권한 migration과 두 Admin 실사용 검증 결과는 `docs/reports/phase-3-implementation.md`를 따른다. Phase 5의 외부 domain·OAuth·기기 smoke test는 별도 확인 항목이다.
...
`npm run verify:phase5`는 Phase 1–4 검사, production build, source·dist·Git history secret/PII 검사, private file 격리, Pages asset hash와 Supabase SQL 계약을 하나의 배포 gate로 실행한다. GitHub Actions도 같은 명령이 통과한 경우에만 Pages artifact를 업로드한다.

production 권한·관계·cycle의 읽기 전용 확인은 `supabase/tests/phase5_security_audit.sql`을 Supabase SQL Editor에서 실행한다. DNS, custom domain, Google OAuth redirect, 계정 제거, key rotation과 rollback 절차는 `docs/reports/phase-5-implementation.md` 및 `supabase/README.md`에 값 없이 기록한다.
