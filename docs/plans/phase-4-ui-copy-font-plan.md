...
| 폰트 | jsDelivr Pretendard CDN | 로컬 asset으로 build에 포함 |
...
- Pretendard 로컬 WOFF2와 license 포함
- 반응형·접근성·긴 문자열 검증
- 관련 단위·정적 검사와 문서 갱신
...
| 국가 | 접힘·펼침 카드 모두 영어 두 글자 코드로 확정 | 편집기에서만 한국어 국가명 표시 | 카드 밀도 |
| 메모 | 검색 포함, 접힌 카드에 6글자 미리보기로 확정 | 펼친 카드에서 전체 내용 표시 | 빠른 회원 식별 |
...
- 빈 값은 `국가 미확인`으로 표시한다.
- 접힌 카드와 펼친 카드 모두 영어 두 글자 코드만 표시한다.
...
- 국가는 한국어 드롭다운으로 선택하고 카드에서는 저장된 영어 코드를 표시
...
`index.html`이 jsDelivr의 Pretendard CSS를 호출하므로 다음 문제가 있다.

- 외부 CDN 장애나 차단에 영향받음
- 실제 build artifact에 font가 포함되지 않음
- 배포 검증이 CDN 상태와 섞임

### 11.2 구현

- 승인된 Pretendard Variable WOFF2를 `src/assets/fonts/`에 추가
- font license 또는 출처 문서를 함께 포함
- `@font-face`를 app CSS에 정의
- `font-display: swap` 사용
- 필요하면 해당 WOFF2만 preload
- jsDelivr stylesheet link 제거
- system font fallback 유지

### 11.3 중단 조건

로컬 font 원본과 재배포 가능한 license를 확인하지 못하면 임의 파일을 저장소에 복사하지 않는다. 이 경우 문구·UI 작업은 완료할 수 있지만 폰트 항목은 별도 미완료로 기록한다.
...
- production Network에서 외부 Pretendard CSS 요청 0건
- local WOFF2 HTTP 200
...
- 로컬 Pretendard가 build artifact에 포함됨
- Phase 1–3 검사와 Phase 4 신규 테스트 통과
- PC·iPhone·iPad 기준 가로 overflow와 console 오류 0건
- GitHub Pages production에서 동일 화면과 local font 확인
